...
public class Parser {
	...
	private static LocalTime[] courseSlotValidStartingTimesMonday = { LocalTime.parse("8:00"), LocalTime.parse("9:00"),
	...
	private static LocalTime[] courseSlotValidStartingTimesTuesday = { LocalTime.parse("8:00"), LocalTime.parse("9:30"),
	...
	// validCourseSlot - Checks the validity of the Course Slot
	private static boolean validCourseSlot(CourseSlot slot) {
		Day day = slot.getDay();
		switch (day) {
		case FR:
			return false;
		case MO:
			return validMondayCourseSlot(slot);
		case TU:
			return validTuesdayCourseSlot(slot);
		default:
			return false;
		}
	}

	private static boolean validTuesdayCourseSlot(CourseSlot slot) {
		return false;
	}

	private static boolean validMondayCourseSlot(CourseSlot slot) {
		return false;
	}
		...
		while (scanner.hasNextLine()) {
			...
			if (scanner.nextLine().equals("Lab slots:")) {
				...
				while (lineStr.equals("") == false) {
					...
					slots.add(
							new LabSlot(getDay(parts[0]), LocalTime.parse(parts[1], DateTimeFormatter.ofPattern("H:m")),
